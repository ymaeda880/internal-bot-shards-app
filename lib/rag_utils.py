...
def build_prompt(
    ...
    sys_inst: str = "あなたは社内アシスタントです。",
    style_hint: str = "standard",
    cite: bool = True,
    strict: bool = True,
) -> str:
    """
    labeled_contexts: ["[S1] 文章...\\n[meta: 2025/a.pdf p.3 / score=0.812]", ...]
    """
    style_map = {
    ...
    if strict:
        # guard = (
        #     "以下のコンテキストに明示された内容【のみ】を根拠として回答してください。"
        #     "コンテキストに根拠が無い事項・数値・手順は推測や常識で補わず、"
        #     "必ず『この資料からは分かりません』と明記してください。"
        #     "コンテキストに含まれていない内容は，回答しないでください．その際には提案や補足もしないでください．"
        #     "不明点は箇条書きで列挙し、どの情報が不足しているかも述べてください。"
        # )
        guard = (
            "以下のコンテキストに書かれていること【のみ】を根拠に回答してください。"
            "もし質問に対応する情報がコンテキストに含まれていなければ、"
            "『この資料からは分かりません』とだけ答えてください。"
            "絶対に一般知識や推測で補足しないでください。"
        )
    else:
        guard = "必要に応じて一般知識で補足しても構いません。"
