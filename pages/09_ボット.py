# pages/09_ボット.py
# ------------------------------------------------------------
# 💬 Internal Bot (RAG, No-FAISS) — 以前のボット風UI + シャード横断検索版（堅牢化）
...
go = go_click or ("send_now" in locals() and send_now)
...
if go and st.session_state.q.strip():
    ...
    shard_dirs = shard_dirs_all if not target_shards else [vs_backend_dir / s for s in target_shards]
        ...
        with st.spinner("検索中…"):
            ...
            for shp in shard_dirs:
                try:
                    ...
                    hits = vdb.search(qv, top_k=K)  # 想定: [(row_idx, score, meta)] or [(score, meta)]
                    for h in hits:
                        # 戻り値の揺れに対応
    ...
    st.info("質問を入力して『送信』を押してください。サイドバーでシャードや回答設定を調整できます。")
